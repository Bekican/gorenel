...
func (c *Client) HealthCheck() bool {
	...
	if err != nil {
		c.logger.Debug("ML health check failed", zap.String("url", c.baseUrl), zap.Error(err))
		return false
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		c.logger.Debug("ML health check returned non-200", zap.Int("status", resp.StatusCode))
		return false
	}
	return true
}
...
func (c *Client) EnsureModelsReady(maxRetries int) {
	...
	// First, wait for ML engine to be reachable
	for attempt := 0; attempt < 30; attempt++ {
		if c.HealthCheck() {
			c.logger.Info("ML engine erişilebilir", zap.String("url", c.baseUrl))
			break
		}
		if attempt == 29 {
			c.logger.Error("ML engine 5 dakika içinde erişilebilir olmadı, vazgeçiliyor",
				zap.String("url", c.baseUrl))
			return
		}
		c.logger.Warn("ML engine henüz hazır değil, bekleniyor...",
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err == nil {
			if len(*stats) > 0 {
				...
				for name, s := range *stats {
					if !s.IsTrained {
						allTrained = false
						c.logger.Warn("Model eğitilmemiş",
							zap.String("model", name),
							zap.Int("attempt", i+1))
					}
				}
				if allTrained {
					c.logger.Info("✅ Tüm ML modelleri hazır ve eğitimli",
						zap.Int("models", len(*stats)))
					return
				}
			}
			c.logger.Warn("ML modelleri eğitimsiz görünüyor, /train çağrılıyor",
				zap.Int("attempt", i+1))
		} else {
			c.logger.Warn("ML model stats okunamadı, /train denenecek",
		...
		cancel()
		if err != nil {
			c.logger.Warn("ML train çağrısı başarısız",
				zap.Int("attempt", i+1), zap.Error(err))
			time.Sleep(10 * time.Second)
			continue
		}
	...
	c.logger.Error("❌ ML modelleri otomatik hazır hale getirilemedi; AI Geçidi devre dışı olabilir")
}
