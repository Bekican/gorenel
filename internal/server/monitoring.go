...
func (m *MonitoringServer) Start(port string) error {
	...
	authMw := func(h http.HandlerFunc) http.HandlerFunc { return h }
	...
	mux.HandleFunc("/metrics", m.corsMiddleware(authMw(rl(m.metricsHandler))))
	mux.HandleFunc("/api/metrics", m.corsMiddleware(authMw(rl(m.metricsHandler))))
	mux.HandleFunc("/info", m.corsMiddleware(m.infoHandler))
	mux.HandleFunc("/analytics", m.corsMiddleware(authMw(rl(m.analyticsHandler))))
	mux.HandleFunc("/api/analytics/realtime", m.corsMiddleware(authMw(rl(m.realtimeAnalyticsHandler))))
...
func (m *MonitoringServer) tunnelPolicyHandler(w http.ResponseWriter, r *http.Request) error {
	...
	// SECURITY: Verify that the requesting user owns this tunnel.
	claims := middleware.GetUserFromContext(r.Context())
	if claims == nil || claims.UserID == "" {
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return nil
	}
	if info, ok := m.tunnelManager.GetTunnelInfo(subdomain); ok && info != nil {
		if info.UserID != claims.UserID {
			m.logger.Warn("Tunnel policy değiştirme reddedildi: sahiplik uyuşmazlığı",
				zap.String("requester", claims.UserID),
				zap.String("owner", info.UserID),
				zap.String("subdomain", subdomain),
			)
			http.Error(w, "Forbidden: you do not own this tunnel", http.StatusForbidden)
			return nil
		}
	}
...
func (m *MonitoringServer) corsMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		isAllowed := false
		if origin != "" {
			// Dinamik host kontrolü (Same-origin ise her zaman izin ver)
			hostURL := "http://" + r.Host
			hostURLSecure := "https://" + r.Host
			if origin == hostURL || origin == hostURLSecure {
				isAllowed = true
...
func (m *MonitoringServer) tunnelsHandlerFunc(w http.ResponseWriter, r *http.Request) {
	...
	// Strict check: if UserID is empty, return nothing
	if claims.UserID == "" {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]interface{}{
			"tunnels": []interface{}{},
	...
	tunnels := m.tunnelManager.GetTunnelsByUser(claims.UserID)
	
	m.logger.Info("Tunnels requested", 
		zap.String("requester_id", claims.UserID), 
		zap.Int("found_count", len(tunnels)),
	)
...
func (m *MonitoringServer) tunnelHistoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	records, err := m.historyStore.ListRecentSessionsByUser(claims.UserID, 100)
	if err != nil {
...
func (m *MonitoringServer) anomaliesHandler(w http.ResponseWriter, r *http.Request) {
	...
	anomalies := m.anomalyStore.GetRecentByUser(claims.UserID, 50)
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]interface{}{
		"anomalies": anomalies,
		"count":     len(anomalies),
...
func (m *MonitoringServer) mlStatsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if m.mlClient != nil {
		...
		if err != nil {
			...
			env.Stats = stats
			// Debug log to confirm stats are flowing
			m.logger.Debug("ML stats retrieved", zap.Int("model_count", len(*stats)))
		}
...
func (m *MonitoringServer) metricsHandler(w http.ResponseWriter, r *http.Request) {
	...
	tunnelCount := 0
	var userRequests, userBytesIn, userBytesOut int64

	if claims != nil && claims.UserID != "" {
		tunnelCount = m.tunnelManager.CountByUser(claims.UserID)
		userRequests, userBytesIn, userBytesOut = m.tunnelManager.GetStatsByUser(claims.UserID)
	}

	metrics := map[string]interface{}{
		"tunnels": map[string]interface{}{
			"active_count": tunnelCount,
		},
		"requests": map[string]interface{}{
			"total":              userRequests,
			"active_connections": atomic.LoadInt64(&ActiveConnections), // Still global but less sensitive
		},
		"bandwidth": map[string]interface{}{
			"bytes_in":  userBytesIn,
			"bytes_out": userBytesOut,
		},
...
func (m *MonitoringServer) realtimeAnalyticsHandler(w http.ResponseWriter, r *http.Request) {
	...
	snapshot := m.analyticsEngine.GetSnapshotByUser(claims.UserID)
	w.Header().Set("Content-Type", "application/json")
...
func (m *MonitoringServer) inspectorHistoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	history := m.inspector.GetHistoryByUser(claims.UserID)
	w.Header().Set("Content-Type", "application/json")
...
// handleDownload serves the CLI binary and checksums
func (m *MonitoringServer) handleDownload(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		m.logger.Warn("File not found", zap.String("file", filePath))
		http.NotFound(w, r)
		return
	}

	m.logger.Debug("Serving file", zap.String("file", filePath))
...
// Uses BACKTICK placeholder because Go raw strings cannot contain backticks.
...
$tempBinary = "$installDir\gorenel_new.exe"
$tempHash = "$installDir\gorenel.sha256"

# Architecture Detection
$arch = if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') { 'arm64' } else { 'amd64' }
$binaryName = "gorenel-windows-$arch.exe"
$downloadUrl = "$gorenelUrl/downloads/$binaryName"

Write-Host ""
Write-Host "[*] Installing Gorenel for Windows ($arch)..." -ForegroundColor Cyan
Write-Host ""

# 1. Download SHA256 checksum
try {
    Write-Host "[>] Fetching checksum..." -ForegroundColor Gray
    Invoke-WebRequest -Uri "$downloadUrl.sha256" -OutFile $tempHash -ErrorAction Stop
} catch {
    Write-Host "[!] Failed to download checksum: $_" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "[!] Re-trying download with alternative method..." -ForegroundColor Yellow
    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        (New-Object System.Net.WebClient).DownloadFile($downloadUrl, $tempBinary)
    } catch {
...
if (Get-Command Get-FileHash -ErrorAction SilentlyContinue) {
    ...
    $expectedHashLine = Get-Content $tempHash | Select-Object -First 1
    $expectedHash = $expectedHashLine.Split(' ')[0].Trim().ToUpper()
    $actualHash = (Get-FileHash $tempBinary -Algorithm SHA256).Hash.ToUpper()
    ...
    if ($expectedHash -ne $actualHash) {
        ...
        Remove-Item $tempHash -ErrorAction SilentlyContinue
        exit 1
    }
...
if (-not $already) {
    ...
    Write-Host "[OK] Added $installDir to User PATH." -ForegroundColor DarkCyan
    try {
...
# Current process: PATH + function so chained commands work
if ($env:Path -notlike "*$installDir*") {
    $env:Path = "$installDir;$env:Path"
}
function global:gorenel { & $binaryPath @args }

# Persist 'gorenel' in PowerShell profile
try {
    if ($PROFILE) {
        ...
        if (-not $hasMarker) {
            ...
            Add-Content -LiteralPath $PROFILE -Value "}" -Encoding UTF8
            Write-Host "[OK] PowerShell profiline 'gorenel' eklendi: $PROFILE" -ForegroundColor DarkCyan
        }
    }
} catch { }
...
Write-Host ""
Write-Host "Restart your terminal or run: . BACKTICK$PROFILE" -ForegroundColor DarkGray
`
...
func (m *MonitoringServer) handleInstallPs1(w http.ResponseWriter, r *http.Request) {
	...
	script = strings.ReplaceAll(script, "BACKTICK", "`")
	w.Header().Set("Content-Type", "text/plain; charset=utf-8")
	w.Write([]byte(script))
}
...
func (m *MonitoringServer) handleTunnelWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	if apiKey == "" {
		m.logger.Warn("Tunnel WebSocket bağlantısı reddedildi: API key yok", zap.String("client_ip", clientIP))
		http.Error(w, "API key required (send X-API-Key header or ?api_key= query param)", http.StatusUnauthorized)
		return
	}
	if m.authManager != nil {
		keyInfo, err := m.authManager.ValidateKey(apiKey)
		if err != nil || keyInfo == nil {
			m.logger.Warn("Tunnel WebSocket bağlantısı reddedildi: Geçersiz API key",
		...
		m.logger.Info("Tunnel WebSocket auth başarılı",
