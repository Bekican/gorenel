...
func (ae *AnalyticsEngine) GetSnapshot() *AnalyticsSnapshot {
	return ae.GetSnapshotByUser("")
}

func (ae *AnalyticsEngine) GetSnapshotByUser(userID string) *AnalyticsSnapshot {
	...
	ae.dataMu.RLock()
	snapshot.TimeSeries = ae.aggregateTimeSeriesByUser(userID, time.Hour)
	for _, dp := range ae.dataPoints {
		if userID != "" && dp.UserID != userID {
			continue
		}
		totalRequests += dp.RequestCount
		responseTimeSum += dp.AvgLatency * time.Duration(dp.RequestCount)
		responseTimeCount += dp.RequestCount

		// Aggregate metadata for this user
		topPaths[dp.Path] += dp.RequestCount
		topCountries[dp.Country] += dp.RequestCount
		topUserAgents[dp.UserAgent] += dp.RequestCount
		statusCodes[dp.StatusCode] += dp.RequestCount
	}
	ae.dataMu.RUnlock()

	snapshot.TotalRequests = totalRequests
	if responseTimeCount > 0 {
		snapshot.AvgResponseTime = responseTimeSum / time.Duration(responseTimeCount)
	}

	// Set filtered top lists
	snapshot.TopPaths = getTopN(topPaths, 10)
	snapshot.TopCountries = getTopN(topCountries, 10)
	snapshot.TopUserAgents = getTopN(topUserAgents, 5)
	snapshot.StatusCodeDist = copyMap(statusCodes)
...
func (ae *AnalyticsEngine) aggregateTimeSeries(duration time.Duration) []TimeSeriesBucket {
	return ae.aggregateTimeSeriesByUser("", duration)
}

func (ae *AnalyticsEngine) aggregateTimeSeriesByUser(userID string, duration time.Duration) []TimeSeriesBucket {
	now := time.Now()
	cutoff := now.Add(-duration)
