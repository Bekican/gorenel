...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) error {
	...
	if r.Method == http.MethodPost {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"token": tokenString, // Fallback için token'ı direkt dönüyoruz
		})
		return nil
	}
	...
	oauthCookieDomain := h.getCookieDomain(r)
...
func (h *AuthHandler) Callback(w http.ResponseWriter, r *http.Request) error {
	...
	dashboardURL := "/dashboard?login=success&t=" + tokenString
	if h.isProd {
		dashboardURL = "https://gorenel.site/dashboard?login=success&t=" + tokenString
	}
...
func (h *AuthHandler) Logout(w http.ResponseWriter, r *http.Request) error {
	...
	cookieDomain := h.getCookieDomain(r)
...
// getCookieDomain resolves the cookie domain consistently based on environment and host.
func (h *AuthHandler) getCookieDomain(r *http.Request) string {
	cookieDomain := ""
	
	// Sadece host gerçekten gorenel.site içeriyorsa domain'i zorla.
	// Aksi takdirde (IP adresi, localhost vs.) boş bırak ki tarayıcı otomatik ayarlasın.
	if h.isProd && strings.Contains(r.Host, "gorenel.site") {
		cookieDomain = ".gorenel.site"
	}
	return cookieDomain
}
...
func (h *AuthHandler) setAuthCookie(w http.ResponseWriter, r *http.Request, tokenString string) {
	...
	logger.Info("Auth cookie set", zap.String("domain", cookieDomain), zap.Bool("secure", h.isProd))
}
