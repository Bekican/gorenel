...
build-client:
	...
	# Linux ARM64
	GOOS=linux GOARCH=arm64 go build ${LDFLAGS} -o bin/${BINARY_NAME_CLIENT}-linux-arm64 cmd/client/main.go
	# macOS AMD64
	...
	# Windows ARM64
	GOOS=windows GOARCH=arm64 go build ${LDFLAGS} -o bin/${BINARY_NAME_CLIENT}-windows-arm64.exe cmd/client/main.go
	@echo "${YELLOW}🔐 Generating SHA256 checksums...${NC}"
	@cd bin && for f in ${BINARY_NAME_CLIENT}*; do \
		if [ -f "$$f" ] && [ "$${f##*.}" != "sha256" ]; then \
			sha256sum "$$f" > "$$f.sha256"; \
		fi \
	done
	@echo "${GREEN}✅ Client built successfully with checksums${NC}"
