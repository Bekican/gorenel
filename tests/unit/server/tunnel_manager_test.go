...
func TestTunnelManager_RegisterAndGet(t *testing.T) {
	...
	tm.RegisterTunnel("test-sub", serverSession, "", 3000, "http://test-sub.gorenel.site:8080")
...
func TestTunnelManager_RemoveTunnel(t *testing.T) {
	...
	tm.RegisterTunnel("remove-me", serverSession, "", 3000, "http://remove-me.gorenel.site:8080")
...
func TestTunnelManager_CustomDomain(t *testing.T) {
	...
	tm.RegisterTunnel("my-sub", serverSession, "api.bekircan.com", 3000, "http://my-sub.gorenel.site:8080")
...
func TestTunnelManager_UpdateStats(t *testing.T) {
	...
	tm.RegisterTunnel("stats-sub", serverSession, "", 3000, "http://stats-sub.gorenel.site:8080")
...
func TestTunnelManager_MultipleTunnels(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		defer s.Close()
		defer c.Close()
		tm.RegisterTunnel(fmt.Sprintf("sub-%d", i), s, "", 3000+i, "http://test.gorenel.site:8080")
	}
...
func TestTunnelManager_ConcurrentRegisterRemove(t *testing.T) {
	...
	for i := 0; i < numTunnels; i++ {
		go func(idx int) {
			...
			tm.RegisterTunnel(sub, s, "", 3000+idx, fmt.Sprintf("http://%s.gorenel.site:8080", sub))
		}(i)
	}
	wg.Wait()
...
func BenchmarkTunnelManager_RegisterTunnel(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		tm.RegisterTunnel(sub, s, "", 3000, "http://bench.gorenel.site:8080")
	}
}
...
func BenchmarkTunnelManager_GetTunnel(b *testing.B) {
	...
	tm.RegisterTunnel("bench-lookup", s, "", 3000, "http://bench.gorenel.site:8080")
