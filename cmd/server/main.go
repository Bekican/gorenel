...
func main() {
	...
	// Ensure ML models are ready in the background (won't block server startup)
	go func() {
		// Wait a bit for the ML engine container to be fully ready
		time.Sleep(10 * time.Second)
		zapLogger.Info("ML modelleri kontrol ediliyor ve gerekirse eğitim başlatılıyor...")
		mlClient.EnsureModelsReady(3)
	}()
...
func handleClient(conn net.Conn, tm *server.TunnelManager, authManager *authmgr.AuthManager, tcpProx *server.TCPProxy, udpProx *server.UDPProxy, historyStore *server.TunnelHistoryStore, reservationRepo *server.PostgresReservationRepository, logger *zap.Logger, cfg *config.Config) {
	...
	if regReq.TunnelType == "tcp" || regReq.TunnelType == "udp" {
		// Port tahsis et (protokol uyumlu tescil için regReq.TunnelType değerini iletiyoruz)
		publicPort, err = tm.AllocatePort(regReq.TunnelType)
		if err != nil {
		...
		subdomain = fmt.Sprintf("%s-%d", regReq.TunnelType, publicPort)
		fullURL = fmt.Sprintf("%s://%s:%d", regReq.TunnelType, cfg.BaseDomain, publicPort)
		...
		if desired != "" {
			...
			// Prevent collision (rare but possible with auto-gen)
			for i := 0; i < 5; i++ {
				if _, ok := tm.GetTunnelInfo(subdomain); !ok {
					break
				}
				subdomain = utils.GenerateSubDomain(8)
			}
	...
	// 3. Pre-compute access token for tunnel isolation
	var accessToken string
	if regReq.TunnelType != "tcp" && regReq.TunnelType != "udp" {
		if strings.TrimSpace(regReq.KeyAuthToken) != "" {
			accessToken = strings.TrimSpace(regReq.KeyAuthToken)
		} else {
			// Backward compatibility: default to public access if not explicitly requested otherwise (nil or true)
			isPublic := true
			if regReq.PublicAccess != nil {
				isPublic = *regReq.PublicAccess
			}
			if !isPublic {
				accessToken = utils.GenerateTunnelToken()
			}
		}
	}

	// 4. Client'a yanıt ver
	...
	if subdomain != "" {
		...
		if accessToken != "" {
			...
			policy.KeyAuthToken = accessToken
			logger.Info("Tunnel access token set",
				zap.String("subdomain", subdomain),
				zap.String("token_prefix", accessToken[:8]+"..."))
		}
		...
		if authKey.UserID == "" {
			logger.Error("Tünel kaydı reddedildi: UserID boş", zap.String("subdomain", subdomain))
			_ = protocol.WriteMessage(conn, protocol.NewErrorMessage(401, "Invalid API key: No UserID associated"))
			return
		}
