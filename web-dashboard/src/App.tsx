...
function App() {
  ...
  // Set isClient to true on mount to signal hydration is complete
  useEffect(() => {
    setIsClient(true);
  }, []);
  ...
  useEffect(() => {
    ...
    if (params.get('login') === 'success') {
      ...
      // Çerez kalkanını aşmak için URL'den gelen token'ı al
      const token = params.get('t');
      if (token) {
        localStorage.setItem('gorenel_token', token);
      }
      ...
      api.getMe()
        ...
        .catch(() => {
          console.error('Failed to fetch user after login success');
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  const clearLocalSession = useCallback(() => {
    ...
    localStorage.removeItem('gorenel_token');
    setUser(null);
    setIsAuthStarted(true);
    setAuthMode('login');
  }, []);
  ...
  useEffect(() => {
    const checkSession = async () => {
      const params = new URLSearchParams(window.location.search);
      if (params.get('login') === 'success') {
        // Skip stored user in local storage to allow fresh login from OAuth redirect
        return;
      }
      try {
        ...
        if (data && data.user) {
          setUser(data.user);
          localStorage.setItem('gorenel_user', JSON.stringify(data.user));
          setIsAuthStarted(true); // Skip landing page for valid sessions
        } else {
          localStorage.removeItem('gorenel_user');
          localStorage.removeItem('gorenel_token');
          setUser(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (tunnelsRes.status === 'fulfilled') {
      ...
      if (tunnelsRes.value.requester_id) setRequesterId(tunnelsRes.value.requester_id);
    }
  ...
  if (currentPath.startsWith('/share/')) {
    const shareId = currentPath.split('/')[2];
        ...
        <div className="mt-auto pt-4 border-t border-white/[0.04]">
          {requesterId && (
            <div className="mb-3 px-3 py-2 bg-white/5 rounded-xl border border-white/10">
              <p className="text-[10px] text-white/30 uppercase font-bold tracking-wider mb-1">Your ID</p>
              <p className="text-[11px] font-mono text-emerald-400/80 truncate" title={requesterId}>
                {requesterId}
              </p>
            </div>
          )}
