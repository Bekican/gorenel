...
export const TunnelsList: React.FC<TunnelsListProps> = ({ tunnels, requesterId, historySessions = [], onOpenConnect, onGoReservations }) => {
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-white/35">
              {tunnels.length} active endpoints 
              {requesterId && <span className="ml-2 text-[10px] bg-white/5 px-1.5 py-0.5 rounded border border-white/10">ID: {requesterId}</span>}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {tunnels.length === 0 ? (
          ...
          tunnels.map((tunnel) => {
            ...
            const isTCPorUDP = tunnel.tunnelType === 'tcp' || tunnel.tunnelType === 'udp' || tunnel.subdomain.startsWith('tcp-') || tunnel.subdomain.startsWith('udp-');
            const keyOn = !isTCPorUDP && !!tunnel.policy?.key_auth_enabled;
            const ipOn = !isTCPorUDP && !!tunnel.policy?.ip_allowlist_enabled;
            const basicOn = !isTCPorUDP && !!tunnel.policy?.basic_auth_enabled;
            const rlOn = !isTCPorUDP && !!tunnel.policy?.rate_limit_enabled;
            return (
              <div
                key={tunnel.id}
              ...
              >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-base truncate max-w-[260px] sm:max-w-none">
                          {isTCPorUDP ? tunnel.publicUrl : `${tunnel.subdomain}.gorenel.site`}
                        </span>
                       ...
                       <div className="flex flex-wrap items-center gap-1.5">
                        {isTCPorUDP ? (
                          <span className="inline-flex items-center gap-1 rounded-md border border-sky-500/15 bg-sky-500/[0.06] px-2 py-0.5 text-[10px] font-medium text-sky-300/80">
                            <Zap className="w-2.5 h-2.5" /> {tunnel.tunnelType?.toUpperCase() || 'TCP/UDP'}
                          </span>
                        ) : (
                          <>
                            {keyOn && (
                ...
                <div className="mt-4 pt-3 border-t border-white/[0.04] flex items-center justify-between text-[10px] text-white/15">
                  <div className="flex items-center gap-3">
                    ...
                    {tunnel.userId && (
                      <span className={`px-1.5 py-0.5 rounded-sm border ${tunnel.userId === requesterId ? 'bg-emerald-500/5 border-emerald-500/10 text-emerald-400/50' : 'bg-rose-500/5 border-rose-500/10 text-rose-400/50'}`}>
                        Owner: {tunnel.userId}
                      </span>
                    )}
                  </div>
