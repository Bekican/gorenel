...
function defaultApiBaseUrl(): string {
  ...
  // gorenel.site ve tüm *.gorenel.site subdomain'leri için API sunucusunu kullan.
  // Caddy /api/* yollarını gorenel-server:9091'e yönlendiriyor, bu yüzden relative path güvenli.
  if (host === 'gorenel.site' || host.endsWith('.gorenel.site')) {
    return '';
  }
...
// Çerezlerin (Cookies) çalışmadığı durumlar için kesin (bulletproof) Authorization header fallback'i
apiClient.interceptors.request.use((config) => {
  if (typeof window !== 'undefined') {
    const token = localStorage.getItem('gorenel_token');
    if (token) {
