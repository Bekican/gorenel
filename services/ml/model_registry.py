...
class ModelRegistry:
    ...
    def predict(self, name: str, X: np.ndarray) -> dict:
        ...
        if not model or not model.is_trained:
            return {'error': f'{name} modeli eğitilmedi'}
    ...
    def predict_compare(self, X: np.ndarray) -> dict:
        """Tüm eğitilmiş modellerden tahmin al ve karşılaştır."""
        results = {}
        for name in self.models:
            if self.models[name].is_trained:
                results[name] = self.predict(name, X)
    ...
    def load_all(self):
        ...
        for name, path in self.model_paths.items():
            try:
                if name == 'autoencoder':
                    # Autoencoder'ın iki dosyası var: _keras ve _meta.pkl
                    if os.path.exists(path + '_meta.pkl'):
